...
def analyze_descriptor_distance_image_pair_with_sift_response(image_0, settings, mask=None,
                                                              plotMatches=True, saveFig=False):
    '''
    Compute distances to the 2nd clostest features and plot histogram + kde
    '''
    ...
    #        kp_img_0 = draw_markers(kp_img_0, detector_kp_0, color=marker_colors, markerSize=1)
        ...
        response_kde_full = st.gaussian_kde(kp_responses,bw_method='silverman')
        response_kde = response_kde_full(x)
        fig1_axes[1].hist(kp_responses, bins=x, color='blue', density=True, alpha=0.4, label='Raw')
        fig1_axes[1].fill_between(x, response_kde, color='red',alpha=0.4)
        fig1_axes[1].set_xlim((0,kp_responses.max()))
        fig1_axes[1].set_xticklabels([])
        #fig1_axes[1].set_ylim([0,max_prob])
        fig1_axes[1].set_ylabel('Prob. density')
        fig1_axes[1].set_xlabel('KeyPoint Response')#fig1_axes[1].scatter(kp_eig_vals[:,0], kp_eig_vals[:,1], color=marker_colors_RGB/255, marker='+')
