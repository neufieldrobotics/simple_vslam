...
contrast_adj = 0.0
img0_name = '/home/vik748/data/Stingray2_test_images/4bit/G0035800_ctrst_adj_{:.2f}.png'.format(contrast_adj)
img1_name = '/home/vik748/data/Stingray2_test_images/4bit/G0035820_ctrst_adj_{:.2f}.png'.format(contrast_adj)
...
def draw_rectangles(image, pts, rect_size=5, color = (128,128,128)):
    out_image = image.copy()
    for pt in pts.astype(int):
        out_image = cv2.rectangle(out_image, tuple(pt-rect_size), tuple(pt+rect_size),color = color,  thickness=2)
    return out_image
    
'''    
resp = []
dist = []
for mat, inlier in zip(matches_01, mask_e_12):
    if inlier == 1: 
        resp.append(kp_0[mat.queryIdx].response)
        dist.append(mat.distance)
    else:
        resp.append(0)
        dist.append(np.NaN)

interest_match_ind = np.argsort(dist)[1] #np.nanargmin(dist)
interest_kp_0 = kp_0[matches_01[interest_match_ind].queryIdx]
interest_kp_1 = kp_1[matches_01[interest_match_ind].trainIdx]

POINT_OF_INTEREST = kp_0[matches_01[interest_match_ind].queryIdx].pt
'''
print ("POINT_OF_INTEREST set to: "+ str(POINT_OF_INTEREST))


kp_0_ind = [i for i,k in enumerate(kp_0) if k.pt == POINT_OF_INTEREST][0]
interest_match_ind = [i for i,m in enumerate(matches_01) if kp_0[m.queryIdx].pt == POINT_OF_INTEREST][0]
interest_kp_0 = kp_0[matches_01[interest_match_ind].queryIdx]
interest_kp_1 = kp_1[matches_01[interest_match_ind].trainIdx]

vmin = np.min(np.hstack((image_0,image_1)))
vmax = np.max(np.hstack((image_0,image_1)))

fig3,axes3 = plt.subplots(2,2, sharex=True, sharey=True,num=39)
fig3.subplots_adjust(left=0.0, bottom=0.0, right=1.0, top=.9, wspace=0.01, hspace=0.1)
fig3.suptitle(det_des_string)
[ax.axis('off') for ax in axes3.flatten()]

patch_size = 22
kp_0_patch_coords = np.array([[int(x-patch_size) for x in interest_kp_0.pt],
                             [int(x+patch_size + 1) for x in interest_kp_0.pt]])
kp_0_patch = image_0[slice(*kp_0_patch_coords[:,1]),slice(*kp_0_patch_coords[:,0])].copy()

kp_1_patch_coords = np.array([[int(x-patch_size) for x in interest_kp_1.pt],
                             [int(x+patch_size + 1) for x in interest_kp_1.pt]])
kp_1_patch = image_1[slice(*kp_1_patch_coords[:,1]),slice(*kp_1_patch_coords[:,0])].copy()

image_0_rect = cv2.rectangle(image_0, tuple(kp_0_patch_coords[0,:]), tuple(kp_0_patch_coords[1,:]), color=(128,128,128))
image_0_mat = draw_rectangles(image_0_rect, kp0_match_01[mask_e_12[:,0]==1], rect_size=5, color = (int(vmin), int(vmin), int(vmin)) )
axes3[0,0].imshow(image_0_mat, cmap='gray',vmin=vmin, vmax=vmax)
axes3[0,0].set_title("Image_0 \ncontrast adj: {:.2f}".format(contrast_adj))

image_1_rect = cv2.rectangle(image_1, tuple(kp_1_patch_coords[0,:]), tuple(kp_1_patch_coords[1,:]), color=(128,128,128))
image_1_mat = draw_rectangles(image_1_rect, kp1_match_01[mask_e_12[:,0]==1], rect_size=5, color = (int(vmin), int(vmin), int(vmin)) )
axes3[0,1].imshow(image_1_mat, cmap='gray',vmin=vmin, vmax=vmax)
axes3[0,1].set_title("Image_1 \ncontrast adj: {:.2f} Matches {}".format(contrast_adj, no_matches))

fig4,axes4 = plt.subplots(2,3, sharex=True, sharey=True,num=40)
fig4.subplots_adjust(left=0.0, bottom=0.0, right=1.0, top=.9, wspace=0.01, hspace=0.1)
[ax.axis('off') for ax in axes4.flatten()]
fig4.suptitle(det_des_string)
axes4[0,0].imshow(kp_0_patch, cmap='gray',vmin=vmin, vmax=vmax)
axes4[0,0].set_title("Image_0 patch\ncontrast adj: {:.2f}".format(contrast_adj))
axes4[0,1].imshow(kp_1_patch, cmap='gray',vmin=vmin, vmax=vmax)
axes4[0,1].set_title("Image_1 patch\ncontrast adj: {:.2f}".format(contrast_adj))

patch_diff = (np.abs(kp_0_patch.astype(np.int16) - kp_1_patch.astype(np.int16)) + vmin).astype(np.uint8)
axes4[0,2].imshow(patch_diff, cmap='gray',vmin=vmin, vmax=vmax)
axes4[0,2].set_title("Diff \nmatch dist: {:.2f}".format(matches_01[interest_match_ind].distance))
