...
def triangulate(T_w_1, T_w_2, pts_1, pts_2, mask=None):
    '''
...
def initialize_3d_plot(number=None, title='Plot', axis_labels=['x', 'y', 'z'],view=[None,None], size=[9.91, 9.89]):
    import sys
    from packaging import version
    fig = plt.figure(number, figsize=size)
    fig.clf()
    ax = fig.add_subplot(111, projection='3d')
    fig.subplots_adjust(0,0,1,1) # Make the plot tight

    if version.parse(sys.modules[plt.__package__].__version__) >= version.Version('3.3.0'):
