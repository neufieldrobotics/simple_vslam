...
def writer(imgnames, masknames, config_dict, queue):
    ...
    if FEATURE_DESCRIPTOR_TYPE == FEATURE_DETECTOR_TYPE:
        Frame.descriptor = Frame.detector
    elif FEATURE_DESCRIPTOR_TYPE == 'orb':
        Frame.descriptor = cv2.ORB_create(**config_dict['ORB_settings'])
        feature_descriptor_config = config_dict['ORB_settings']
    elif FEATURE_DESCRIPTOR_TYPE == 'zernike':
            ...
            if USE_GTSAM:
                ...
                if fr_curr.frame_id % GTSAM_INTERVAL ==0:
                    try:
                        ...
                        e = sys.exc_info()[0]
                        vslog.fatal("Exception occurered in factor_graph update: {}".format(e))
                        while(True):
